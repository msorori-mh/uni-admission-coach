...
const AdminPayments = () => {
  ...
  const fetchData = async () => {
    const [{ data: r }, { data: s }, { data: p }, { data: m }] = await Promise.all([
  ...
  const getPlanName = (planId: string | null) => planId ? plans.find((p) => p.id === planId)?.name || "-" : "-";
  ...
  const handleReview = (req: PaymentRequest) => {
  ...
  const handleApprove = async () => {
    ...
    if (selectedRequest.subscription_id) {
      const plan = selectedRequest.plan_id ? plans.find((p) => p.id === selectedRequest.plan_id) : null;
      const now = new Date();
      // Default to plan duration or 5 months
      let durationMonths = 5;
      if (plan) {
        // fetch full plan data
        const { data: fullPlan } = await supabase.from("subscription_plans").select("duration_months").eq("id", plan.id).single();
        if (fullPlan) durationMonths = fullPlan.duration_months;
      }
  ...
  const handleReject = async () => {
    ...
    if (error) toast({ variant: "destructive", title: error.message });
      ...
      if (selectedRequest.subscription_id) {
        await supabase.from("subscriptions").update({ status: "cancelled" }).eq("id", selectedRequest.subscription_id);
      }
    ...
    <AdminLayout>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {req.amount.toLocaleString()} {req.currency} • {getPlanName(req.plan_id)} • {getMethodName(req.payment_method_id)}
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1">
                      {req.receipt_url && (
                        <Button variant="ghost" size="icon" onClick={() => handleViewReceipt(req)}>
                          <ImageIcon className="w-4 h-4" />
                        </Button>
                      )}
                      {req.status === "pending" && (
                        <Button variant="ghost" size="icon" onClick={() => handleReview(req)}>
          ...
          {selectedRequest && (
            <div className="space-y-4">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">الخطة:</span><span className="font-medium">{getPlanName(selectedRequest.plan_id)}</span></div>
      ...
      <Dialog open={receiptDialog} onOpenChange={setReceiptDialog}>
        <DialogContent className="max-w-lg">
          ...
          {selectedRequest?.receipt_url && (
            <img src={selectedRequest.receipt_url} alt="سند الدفع" className="w-full rounded-lg" />
          )}
        </DialogContent>
      </Dialog>
    </AdminLayout>
