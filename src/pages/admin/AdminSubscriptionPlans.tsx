...
const AdminSubscriptionPlans = () => {
  ...
  useEffect(() => {
    ...
    supabase.from("subscription_settings").select("*").limit(1).then(({ data }) => {
  ...
  const handleSave = async () => {
    ...
    const { error } = settingsId
      ? await supabase.from("subscription_settings").update(payload).eq("id", settingsId)
      : await supabase.from("subscription_settings").insert(payload);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="مثال: اشتراك لفترة القبول" />
            </div>
